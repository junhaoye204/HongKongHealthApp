...
struct HomeView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .sheet(isPresented: $showMoodTracker) { MoodTrackerView() }
            .sheet(isPresented: $showShareSheet) { ShareSheet(activityItems: [generateShareText()]) }
            .onAppear {
                withAnimation(.spring(response: 0.9, dampingFraction: 0.8)) { animateHeader = true }
                localStore.resetDailyData()
                startMotivationTimer()
                startTipTimer()
            }
            .onDisappear {
                motivationTimer?.invalidate(); motivationTimer = nil
    ...
    private var miniDashboard: some View {
        MiniDashboardView(healthManager: healthManager).padding(.horizontal)
    }

    private var waterIntakeSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                Text("今日飲水進度").font(.headline)
                Spacer()
    ...
    private func tipCardView(for idx: Int) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("每日健康提示").font(.caption).foregroundColor(.secondary)
            Text(tips[idx]).font(.body.bold())
        }
        .padding()
    ...
    private var weeklyStepsChartSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                Text("本週步數趨勢").font(.headline)
                Spacer()
                Button("詳情") { showWeeklyChart.toggle() }
                    .font(.subheadline.bold()).foregroundColor(.white)
                    .padding(.vertical, 6).padding(.horizontal, 12)
                    .background(Color.purple.gradient).clipShape(Capsule())
    ...
    private func cardView(for idx: Int) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("每日鼓勵").font(.caption).foregroundColor(.secondary)
            Text(motivations[idx]).font(.body.bold())
        }
        .padding()
        .frame(maxWidth: .infinity, alignment: .leading)
        .background(
    ...
    private var recentMeals: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("最近餐單").font(.headline).padding(.horizontal)
